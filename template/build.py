...
<head>
    ...
    <title>Prezentacja Interaktywna</title>
    
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Asap:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400;1,600&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
...
{header_comp}
    ...
    <button class="nav-overlay-btn prev-slide" id="overlayPrevBtn" title="Poprzedni slajd (←)">
        <i class="fa-solid fa-chevron-left"></i>
    </button>

    <!-- Przycisk Boczny Następny -->
    <button class="nav-overlay-btn next-slide" id="overlayNextBtn" title="Następny slajd (→ / Spacja)">
...
def push_to_github():
    try:
        print("\n[GIT] Wysyłanie zaktualizowanego pliku index.html na GitHub...", flush=True)
        subprocess.run(["git", "add", "index.html"], check=True, cwd=base_dir)
        subprocess.run(["git", "commit", "-m", "Auto build: update index.html"], capture_output=True, cwd=base_dir)
        push_res = subprocess.run(["git", "push", "origin", "main"], capture_output=True, text=True, cwd=base_dir)
        if push_res.returncode == 0:
            print("[GIT SUKCES] Wysełano index.html do GitHub!", flush=True)
        else:
            print(f"[GIT NOTICE] {push_res.stderr.strip() or push_res.stdout.strip()}", flush=True)
    except Exception as e:
        print(f"[GIT WARNING] Błąd wysyłania do GitHuba: {e}", flush=True)
